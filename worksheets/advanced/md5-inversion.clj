...
;; **
;;; So, to sum up: in a probabilistic programming language that allows the creation of arbitrary distributions,  conditioning on these distributions, and the use of arbitrary primitives, it must be possible to write programs like this. Programs that sample from a huge space of possibilities, and place a hard constraint - a Dirac observation - on one particular result. Such programs do not play to the strengths of probabilistic programming languages; they are more accurately understood as problems of satisfiability rather than problems of inference.
;;; 
;;; This is, in fact, why the Dirac distribution doesn't come prepackaged within Anglican - to discourage the writing of precisely this kind of program. In general, probabilistic programming is not intended for solving problems whose primary difficulty is in satisfying hard constraints. Now, there are some situations in which Dirac observations are perfectly reasonable things to do. But if you choose to do so, think of this worksheet - and make sure you're not doing something stupid.
;; **
